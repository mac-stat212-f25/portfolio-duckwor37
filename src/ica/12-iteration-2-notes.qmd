...
```{r}
mtcars_vector <- vector("character", c(ncol(mtcars)))

for (i in seq_along(mtcars)) {
  mtcars_vector[i] <- class(mtcars[[i]])
}
...
```{r purrr_col_classes_error, error=TRUE, eval = FALSE}
...
```{r}
diamond_summ_stats <- vector("list", length = ncol(diamonds_num))

for (i in seq_along(diamonds_num)) {
  diamond_summ_stats[[i]] <- summ_stats(diamonds_num[[i]])
} 
...
```{r}
string_data2 <- tibble(
  string = c("eye", "leg", "feet", "hands"),
  start = c(1, 2, 3, 2),
  end = c(1, 5, 4, 9)
)

pmap_chr(string_data2, str_sub)
...
pmap_chr(string_data3, str_sub, string = "Hello World")
```
...
df_arg_combos |> 
  mutate(fit_mod = pmap(df_arg_combos, fit_model, data = diamonds))
