...
```{r}
df_arg_combos <- crossing(
  combos = c(price ~ carat, price ~ carat + cut, price ~ carat + cut + clarity, price ~ carat + cut + clarity + color), 
...
```{r eval=FALSE}
fit_model <- function(data, combos, remove_outliers) {
...
```{r}
vec_for_mod <- vector("list", nrow(df_arg_combos))

for (i in seq_along(vec_for_mod)) {
  this_formula <- df_arg_combos$combos[[i]] # Double [[ for the **list** of formulas
